...
impl StoreCacheDataHandler {
    ...
    pub fn compress_and_store_file(&self, path: &Path, hash_str: &str) -> CacheHandlerResult<u64> {
        ...
        let mut input = BufReader::new(
            File::open(path)
                .map_err(|e| CacheHandlerError::FailedToOpenFile(e, "file to compress"))?,
        );
        let output =
            BufWriter::new(File::create(destination_path).map_err(|e| {
                CacheHandlerError::FailedToOpenFile(e, "compressed file destination")
            })?);
    ...
    pub fn uncompress_and_restore(
        ...
        let input = BufReader::new(File::open(source_file).map_err(|e| {
            CacheHandlerError::FailedToOpenFile(e, "compressed file to uncompress")
        })?);
        let mut output =
            BufWriter::new(File::create(destination_path).map_err(|e| {
                CacheHandlerError::FailedToOpenFile(e, "restored file destination")
            })?);
        // Wrap the stdout writer in a Snappy writer.
